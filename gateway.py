...
class VKLogin(object):
    ...
    def checkData(self):
        if not self.engine.token and self.password:
            ...
            self.engine.login_by_password()
            self.engine.confirm_app()
        ...
        if not self.engine.captcha and self.Online:
            try:
                ...
                if self.engine.lastMethod[0] == "messages.send":
                    msgSend(Component, self.jidFrom, _("You're not allowed to perform this action."),
    ...
    def captchaChallenge(self):
        if self.engine.captcha:
            ...
            if imgData:
                ...
                obTag = msg.setTag("data",
                                   {"cid": "sha1+%s@bob.xmpp.org" % imgHash, "type": "image/jpg", "max-age": "0"},
                                   xmpp.NS_URN_OOB)
...
class tUser(object):
    ...
    def sendInitPresence(self):
        ...
        logger.debug(
            "tUser: sending init presence to %s (friends %s)" % (self.jidFrom, "exists" if self.friends else "empty"))
